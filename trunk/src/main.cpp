...
int find_vector_X(const double mattr_array[], const int size, double vector_x[])
{
    int counter = 0;
    for (int i=0; i<size; ++i)
    {
        for (int j=0; j<size; ++j)
        {
            ...
            {
                res += (mattr_array[k * size + j] * mattr_array[i * size + k]);
            }
...
int main()
{
	...
	{
		cout<<"------------------------------------------------------------------------------------------------------"<<endl;
		cout<<"Enter dimention of the matrix 'A' (less than 50):";
		cin>>A_size;
		...
		if(validate_dimension(A_size))
		{
			...
			cout<<endl<<"Transforming matrix"<<endl;
			transform_mattrix(A, A_size);
			cout<<"Matrix 'A':"<<endl;
			print_mattrix(A, A_size);


			cout<<endl<<"Calculating X vector"<<endl;
			find_vector_X(A,A_size,X);
			cout<<"Vector 'X':"<<endl;
			print_mattrix(X, A_size*A_size, 1, A_size*2);

			cout<<endl<<"Calculating sum"<<endl;
			U = calculate_sum(X, A_size*A_size);
			cout.setf(ios::fixed,ios::floatfield);
			cout.precision(4);
			cout<<"Sum 'U':"<<U<<endl;
		}
